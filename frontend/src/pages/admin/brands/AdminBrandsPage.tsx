    ...
    <AdminLayout title="Brands" backHref="/admin" backLabel="← Dashboard">
      <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8 space-y-4 sm:space-y-6">
        <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 border border-gray-100">
          <h2 className="text-base sm:text-lg font-bold text-primary-900 mb-4">Add Brand</h2>
                ...
                <tbody className="divide-y">
                  {data?.results.map((brand) => (
                    <tr key={brand.id} className="hover:bg-gray-50">
                      <td className="px-4 sm:px-6 py-3 text-primary-900 font-medium text-xs sm:text-sm">{brand.name}</td>
                      <td className="px-4 sm:px-6 py-3 text-gray-600 text-xs sm:text-sm">{brand.slug}</td>
