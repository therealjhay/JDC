...
export default function ProductCard({ product }: Props) {
  ...
  return (
    <Link to={`/products/${product.id}`} className="group focus-visible:ring-2 focus-visible:ring-navy-400 focus-visible:ring-offset-2 rounded-xl">
      <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 relative">
        <div className="relative h-64 bg-gray-100">
          ...
          <div className="absolute inset-0 flex items-end justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="bg-navy-900/90 text-white text-xs font-semibold px-3 py-2 rounded-full">
              Quick view →
            </span>
          </div>
        </div>
        <div className="p-4">
          ...
          <h3 className="mt-1 text-navy-900 font-semibold text-lg line-clamp-2 group-hover:text-navy-600 transition-colors">
            {product.name}
          </h3>
          <p className="mt-2 text-navy-700 font-bold text-xl">
            From {formattedPrice}
          </p>
          <div className="mt-2">
